...
var DKEY = 'pw.draft'
function wipeDraft() { try { localStorage.removeItem(DKEY) } catch { /* ignore */ } }
/* these assertions use the origin's real storage, so put back whatever was there */
var __savedDraft = null
try { __savedDraft = localStorage.getItem(DKEY) } catch { /* ignore */ }
...
try { localStorage.setItem(DKEY, '{not json') } catch { /* ignore */ }
eq(readDraft(), null, 'DF11. a corrupt entry reads as no draft')
try { localStorage.setItem(DKEY, JSON.stringify({ at: 1 })) } catch { /* ignore */ }
