...
export function initLayout() {
  ...
  function onMove(e) {
    ...
    try { localStorage.setItem(SPLIT_KEY, String(w / r.width)); } catch (err) {}
    if (editor) editor.layout();
  }
  function stop() {
    if (!dragging) return;
  ...
  on(splitterEl, "dblclick", () => {
    ...
    try { localStorage.setItem(SPLIT_KEY, "0.46"); } catch (err) {}
    if (editor) editor.layout();
  });
  ...
  function onMoveV(e) {
    ...
    try { localStorage.setItem(DRAWER_KEY, String(drawerRatio)); } catch (err) {}
