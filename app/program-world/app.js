...
export async function runCode() {
  ...
  try {
    ...
    setStatus(error ? "error" : "ready",
      error ? "Finished with " + error.type
        : limit ? (limit.kind === "steps"
...
setOutputHandler(function (produced) {
  ...
  produced.forEach(function (p) {
    ...
    if (existing.kind === "bin") { existing.bytes = bytes; }
    else {
...
async function runAsGame() {
  ...
  if (leaveAfterGame) {
    ...
    leaveAfterGame = false;
    hideGame();
  } else if (res.stopped) { setGameMsg("Stopped at the end of the frame."); setStatus("ready", "Game stopped"); }
  else if (res.error) {
