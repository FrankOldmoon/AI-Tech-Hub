...
function build() {
  ...
  if ($('viewTitle')) {
    $('viewTitle').textContent = v.kind === 'turtle'
      ? 'Turtle drawing'
      : v.kind === 'frames'
        ? 'Animation (' + (v.frames || []).length + ' frames)'
...
export function initViews() {
  ...
  on(document, 'keydown', function (e) {
    ...
    if (e.key === 'Escape') { e.preventDefault(); closeViews() } else if (e.key === ' ') { e.preventDefault(); if (playing) stop(); else play() }
  }, true)
