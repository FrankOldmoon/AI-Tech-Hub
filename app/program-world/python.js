...
function onMessage(ev) {
  ...
  if (msg.type === "ready") {
    ...
    if (!msg.id) { if (ready) ready.reject(err); poison(); }   // the runtime itself failed
    else { settle("err", err); poison(); }
    ...
    if (msg.produced && msg.produced.length && outputHandler) {
      try { outputHandler(msg.produced); } catch (e) { /* the run still counts */ }
    }
    if (msg.data && msg.data.views && msg.data.views.length && viewHandler) {
      try { viewHandler(msg.data.views); } catch (e) { /* the trace still counts */ }
    }
    settle("ok", msg.data);
  }
}
