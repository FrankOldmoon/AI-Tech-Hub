...
export const demos: Demo[] = [
  ...
  },
  {
    ...
    description: { zh: '六步演示：输入 → 灰度化 → 降噪 → 增强 → 特征强调 → 处理结果。同一张图逐步被改造，每一步都并排对比原图与这一步的效果。', en: 'A six-step walkthrough: input → grayscale → noise reduction → enhancement → feature emphasis → processed. One image changes step by step, each step shown side by side with the original.' },
    howItWorks: { zh: '教学页：每一步的输入都是上一步的输出，所以「顺序」本身就是知识点（先降维、先降噪再增强、最后提特征）。算子全部复用图像处理工坊的注册表（Canvas 与 OpenCV），不另写算法；链是固定的，六步一次并排呈现，前三个 canvas 算子立即出结果，第 5 步的 OpenCV 加载完成后自动补上。', en: 'A lesson page: each step consumes the previous step’s output, so the order is itself the point (reduce dimensions, denoise before enhancing, extract features last). Operators are reused from the image workbench registry (Canvas and OpenCV) — the chain is fixed and all six steps are shown side by side; the three canvas operators run instantly and OpenCV fills in step 5 once loaded.' },
    icon: 'i-lucide-git-branch',
    status: 'ready',
    classroomSafe: true,
