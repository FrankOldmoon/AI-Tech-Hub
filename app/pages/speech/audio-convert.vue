...
/**
 * 音频格式转换：导入音频 → 在 MP3 / WAV / OGG / M4A / FLAC 之间互转 → 对比体积并下载。
 * 转换在浏览器内由 ffmpeg.wasm 完成（见 ~/utils/ffmpeg），文件不上传。
 */
const { t } = useI18n()
const { getDemo } = useDemos()
const demo = computed(() => getDemo('speech', 'audio-convert')!)
...
function onMeta(e: Event) {
  ...
  sourceMeta.value = formatTime(el.duration)
}
</script>
